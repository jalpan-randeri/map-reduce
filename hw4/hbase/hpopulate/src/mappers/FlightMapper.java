...
public class FlightMapper extends Mapper<Object, Text, ImmutableBytesWritable, Writable> {
    ...
    protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if(isValid(tokens)) {
            ...
            String delay = tokens[FlightConts.INDEX_DELAY];
            String diverted = tokens[FlightConts.INDEX_DIVERTED];
            String cancelled = tokens[FlightConts.INDEX_CANCELED];
            String month = tokens[FlightConts.INDEX_MONTH];
            String year = tokens[FlightConts.INDEX_YEAR];

            Put row = new Put(Bytes.toBytes(row_key));
            row.add(TableConts.COLUMN_FAMILY.getBytes(), TableConts.COLUMN_CANCELLED.getBytes(), cancelled.getBytes());
            row.add(TableConts.COLUMN_FAMILY.getBytes(), TableConts.COLUMN_DIVERTED.getBytes(), diverted.getBytes());
            row.add(TableConts.COLUMN_FAMILY.getBytes(), TableConts.COLUMN_DELAY.getBytes(), delay.getBytes());
            row.add(TableConts.COLUMN_FAMILY.getBytes(), TableConts.COLUMN_MONTH.getBytes(), month.getBytes());
            row.add(TableConts.COLUMN_FAMILY.getBytes(), TableConts.COLUMN_YEAR.getBytes(), year.getBytes());
